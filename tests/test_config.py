...
def test_carrega_contrato_do_projeto(contrato_real):
    ...
    inscricoes = contrato_real.datasets["inscricoes"]
    assert inscricoes.chave == ("id_inscricao",)
    assert "cnpj" in inscricoes.sensiveis
...
def test_referencia_para_dataset_inexistente_e_recusada(tmp_path, contrato):
    contrato["datasets"]["inscricoes"]["regras"].append(
        {"tipo": "referencia", "coluna": "id_inscricao", "dataset": "municipios"}
    )
    with pytest.raises(ErroConfig, match="dataset 'municipios'"):
        carregar(escrever_contrato(tmp_path, contrato))
